...
class WebRtcStatsLogger(
    ...
    private fun processReport(report: RTCStatsReport) {
        ...
        // 2. Candidate Pair & Candidate Types
        statsMap.values.filter { it.type == "candidate-pair" && it.members["state"] == "succeeded" }
            .forEach { stats ->
                ...
                val remoteId = stats.members["remoteCandidateId"] as? String
                val localType = localId?.let { statsMap[it]?.members?.get("candidateType") } ?: "unknown"
                val remoteType = remoteId?.let { statsMap[it]?.members?.get("candidateType") } ?: "unknown"
                
                sb.append("CONN: ${localType} -> ${remoteType} | RTT: ${(rtt * 1000).toInt()}ms | Available: ${availableBitrate.toInt()} kbps\n")
            }
        ...
        statsMap.values.filter { it.type == "inbound-rtp" }.forEach { stats ->
            ...
            val codec = codecId?.let { statsMap[it]?.members?.get("mimeType") } ?: "unknown"
            ...
            if (kind == "video") {
                ...
                sb.append("IN VIDEO: ${width}x${height} | Codec: $codec | Decoded: $framesDecoded | Dropped: $framesDropped | Lost: $packetsLost | Jitter: ${(jitter * 1000).toInt()}ms\n")
            } else if (kind == "audio") {
                sb.append("IN AUDIO: Codec: $codec | Lost: $packetsLost | Jitter: ${(jitter * 1000).toInt()}ms\n")
            }
        }
        ...
        statsMap.values.filter { it.type == "outbound-rtp" }.forEach { stats ->
            ...
            if (kind == "video") {
                ...
                val width = stats.members["frameWidth"] as? Long ?: 0
                val height = stats.members["frameHeight"] as? Long ?: 0
                sb.append("OUT VIDEO: ${width}x${height} | Codec: $codec | Frames: $framesSent\n")
            } else if (kind == "audio") {
                sb.append("OUT AUDIO: Codec: $codec\n")
            }
        }
